// ==UserScript==
// @name        SRES pivot reporter
// @author      Damon Ellis and Wen-Chen Hol
...
// @downloadURL https://github.com/damonkellis/canvascode/raw/pivoted/SRESreport.user.js
...
function createOntaskCSV() {
 ...
 // if (debug) { console.log(tmpId); }
        ...
        if ( fields.indexOf( tmpCode + "_views" ) <0 ) {
            ...
            tmpTitle = tmpTitle.replace(punctRE, '').replace(spaceRE, ' ');
            //console.log( tmpTitle );
            if ( titleAr.indexOf( tmpTitle + "_views" )>-1 ){
                tmpTitle = tmpTitle + " " + tmpCode + " "
            }
            titleAr.push( tmpTitle + "_views" ) ;
            titleAr.push( tmpTitle + "_participations" );
        ...
        ontaskReportAr[tmpId][tmpCode + "_participations"]= 0 ;
...
(function () {
  ...
  function addAccessReportButton() {
    ...
    if ($('#jj_ontask_access_report').length === 0) {
      $('#people-options > ul').append('<li class="ui-menu-item" role="presentation" tabindex="-3"><a id="jj_ontask_access_report" class="ui-corner-all" role="menuitem"><i class="icon-analytics"></i> OnTask access data</a></li>');
      $('#jj_ontask_access_report').one('click', {
        type: 2
      }, accessReport);
    }
